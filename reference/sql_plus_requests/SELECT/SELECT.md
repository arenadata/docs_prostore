...
переменную. Если для ключевого слова указано значение 0, то пропускается 0 строк — это равносильно запросу 
без ключевого слова `OFFSET`. 

Запросы с ключевым словом `OFFSET` без ограничения количества строк не поддерживаются. То есть, если ключевое 
`OFFSET` указано в запросе, то перед ним должно быть ключевое слово `LIMIT <>` или `FETCH NEXT <> ROWS ONLY`. 
Обратного ограничения нет: ключевые слова `LIMIT <>` или `FETCH NEXT <> ROWS ONLY` можно использовать без `OFFSET`.

Ключевое слово `OFFSET` рекомендуется сочетать с `ORDER BY` для получения упорядоченного набора строк. Ключевое слово 
