...
(например, при неполадках в сети или СУБД) операция останется в статусе «Ошибка». В этом случае необходимо устранить 
причины, по которым операция не была успешно завершена, и возобновить её обработку с помощью запроса 
...
неуспешный — исключение. Пустая строка в ответе означает, что в горячей дельте нет операций со статусом «Ошибка».
