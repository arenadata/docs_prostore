...
*   объект ResultSet с одной записью, содержащей результат проверки, при успешном выполнении запроса. Результат 
    включает список проверенных СУБД хранилища, а также сообщение об успешной проверке или найденном расхождении;
...
```sql
CHECK_DATA(sales.sales, 4, [identification_number, transaction_date, product_code])
```
...
1. В каждой из целевых СУБД:
    ...
    3. По каждой дельте — в зависимости от наличия столбцов в запросе — рассчитывается количество или 
       контрольная сумма загруженных записей. Порядок расчета контрольной суммы см. [ниже](#checksum).
2. Значения по каждой дельте сравниваются в целевых СУБД.
3. Если одно из значений отсутствует или не соответствует другим, в ответе возвращается сообщение о 
