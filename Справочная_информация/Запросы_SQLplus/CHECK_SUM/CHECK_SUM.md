...
Расчет происходит пошагово: по логической таблице и затем, если требуется,
по всей логической базе данных. Полученные значения сравниваются между целевыми СУБД. Если контрольные суммы 
на каком-либо из шагов различаются между СУБД, в ответе возвращается исключение с информацией о том, 
что найдено расхождение в данных.
...
проверяемой логической таблицы или логической базы данных.
...
![](check_sum_по_таблице_целиком.png)
{: .figure-center}
*Ответ CHECK_SUM по всей таблице*
{: .figure-caption-center}
...
*   `delta_num` — номер открытой (горячей) или закрытой [дельты](../../../Обзор_понятий_компонентов_и_связей/Основные_понятия/Дельта/Дельта.md), 
    по которой рассчитывается контрольная сумма. Если других аргументов нет в запросе, контрольная сумма 
...
1. По каждой таблице логической БД в указанной дельте рассчитывается контрольная сумма, как описано выше <>.
2. Контрольные суммы по таблицам записываются через точку с запятой. 
3. Далее порядок такой же, как для таблицы (см. пункты 3-5 выше).
...
сумму: 1650746722 (см. пример расчета в разделе по `CHECK_DATA`) и 808792881.
