...
влияет на порядок маршрутизации запроса (см. раздел [Маршрутизация запросов к данным материализованных представлений](../../../Работа_с_системой/Запрос_данных/Маршрутизация_запросов_к_данным/Маршрутизация_запросов_к_данным.md#sect_mat_view_routing)).
...
* `ORDER BY <column_name> OFFSET M` — возвращается N упорядоченных строк, начиная со строки с номером M+1;
* `ORDER BY <column_name> LIMIT N OFFSET M` — возвращается N упорядоченных строк, начиная со строки с номером M+1
  (см. пример [ниже](#ex_offset_limit_and_order_by)). 
  Такое сочетание ключевых слов позволяет выбирать данные порционно с сохранением их порядка.
...
с `OFFSET` следует добавлять ключевое слово `ORDER BY`.
